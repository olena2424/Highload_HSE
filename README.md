...
1. Для значения DAU берём среднее значение из найденного ранее диапазона - 920 млн (как 48.4% от MAU 1.9 млрд).
...
Тип запроса | Средний RPS | Пиковый RPS (x2.5) | Метод расчёта
:--- | :---: | :---: | :---:
Просмотр ленты | ~98 000 | ~245 000 | 920M DAU * 9 сессий / 86 400 сек.
Просмотр видео | ~3 655 000 | ~9 138 000 | 920M DAU * 343 видео / 86 400 сек.; это поток просмотров/событий, а не синхронная выдача видео из API, так как сам видеоконтент отдаётся CDN.
Публикация видео | ~266 | ~665 | 23 млн видео / 86 400 сек. = 266 RPS
Лайк / Комментарий | ~160 000 | ~400 000 | 920M DAU * 15 действий / 86 400 сек.
Сбор статистики просмотров | ~3 655 000 событий/с | ~9 138 000 событий/с | По одному событию просмотра на просмотр видео.
...
Для расчётов взят ДЦ в Северной Америке, который принимает около 25% глобального API и upload-трафика. Видеотрафик download не концентрируется в одном ДЦ: он распределяется по CDN edge/PoP-узлам, поэтому для центрального ДЦ отдельно учитывается только origin/cache-miss поток.
...
Nwork = 167 666 / 50 000 = 3,35 = 4

Применяем формулу резервирования N + 1 (один дополнительный узел для отказоустойчивости):
Ntotal = 5

Итого для уровня L7 нужно 5 балансировщиков nginx, работающих в режиме active‑active. При отказе любого одного из них оставшиеся 4 продолжат обслуживать пиковую нагрузку (4 * 50 000 = 200 000 RPS, что выше требуемых 167 666).

#### Расчёт количества L4 балансировщиков

Ограничивающие факторы:
- API-трафик: средний размер HTTP-запроса к API составляет около 1–2 КБ. Пиковый трафик API на L4 (ДЦ США): 167 666 RPS * 2 КБ ≈ 335 МБ/с ≈ 2,7 Гбит/с.
- Видеопоток (Download): пиковый глобальный трафик ~1 402 000 Гбит/с. Пользовательская раздача видео обслуживается распределённой CDN-сетью: edge/PoP-узлами и кешами у провайдеров. Центральный ДЦ выступает как origin shield и получает только cache-miss/refill поток. Для расчёта примем cache-miss 0,1% от регионального пика США (~350 500 Гбит/с), то есть ~350 Гбит/с. Этот поток обслуживается CDN/S3-контуром и не проходит через основной L4 API-балансировщик.
- Upload-трафик: 25% от глобального пикового Upload (~51 Гбит/с) = ~13 Гбит/с на ДЦ США. Тяжёлый upload идёт напрямую в объектное хранилище по pre-signed URL; через API-балансировщики проходит только выдача URL и запись метаданных. Если считать общий ingress-контур ДЦ, upload добавляет около 13 Гбит/с.
- Итого через основной L4 API-контур: ≈ 2,7 Гбит/с. Для общего ingress-контура API + Upload: ≈ 2,7 + 13 ≈ 15,7 Гбит/с.

Один L4-балансировщик с 40-гигабитным интерфейсом легко обрабатывает такой поток. Для покрытия 15,7 Гбит/с достаточно одного рабочего узла. Применяем схему active‑active с N+1 резервированием, N ≥ 2 (минимум 3 узла):
...
- При выходе одного узла оставшиеся 2 покрывают расчётные 15,7 Гбит/с.
...
Публикация видео (метаданные) | Video (вставка), video_user_lookup (вставка) | Запись | 266 | 665
Fan-out публикаций в ленты подписчиков | Feed_subscription (асинхронная вставка через Kafka) | Запись | ~13 300 | ~33 300
...
Feed_subscription | ~33 300 (асинхронный fan-out публикаций) | 245 000
Like | 400 000 | —
Comment | 125 000 | —
...
- Пиковая нагрузка на синхронный сервис выдачи ленты составляет около 245 тыс. RPS глобально. Поток просмотров и статистики значительно выше: до ~9,1 млн событий/с, но он обрабатывается отдельно от Feed Service, через асинхронный контур сбора событий: Interaction Service -> Redis/Kafka -> Cassandra/ScyllaDB и агрегаторы счётчиков.
...
Ограничения: пиковая нагрузка около 665 видео/с глобально; для ДЦ США при доле 25% это около 166 видео/с.
...
Nginx | L7-балансировщик, SSL termination, маршрутизация по поддоменам | Согласно п. 4.2, для ДЦ в США требуется 5 L7-балансировщиков с совокупной производительностью 250 000 RPS. Nginx на 8-ядерных серверах с включённым SSL достигает устойчивых 50 000 RPS на экземпляр. При отказе одного узла оставшиеся 4 покрывают расчётные 167 666 RPS. Nginx также поддерживает продвинутое кеширование ответов и лимитирование запросов.
HAProxy / LVS | L4-балансировщики для распределения TCP-соединений между L7-узлами | В схеме active‑active с VRRP/ECMP обеспечивается отказоустойчивость входного уровня. Один L4-балансировщик с 40-гигабитным интерфейсом покрывает расчётные ~15,7 Гбит/с API + Upload ingress на ДЦ. HAProxy имеет малый overhead и высокую надёжность.
...
L7-балансировщики (Nginx) | Active‑active, 5 узлов, N+1 | Автоматическое исключение отказавшего узла из upstream; перераспределение трафика | RTO < 10 с
...
- **L4/L7 балансировка внутри ДЦ** – три L4‑балансировщика в режиме active‑active (N+1, N≥2) распределяют TCP‑соединения между пятью L7‑балансировщиками Nginx (active‑active). L7 выполняет SSL‑терминацию и маршрутизацию по поддоменам: `api`, `upload`, `video`, `static`. Видеоданные при просмотре не идут через этот контур: их раздаёт CDN.
...
- Лайки, комментарии и подписки через API Gateway попадают в **Interaction Service**, который инкрементирует соответствующие поля в Redis `video:metrics:{video_id}` и публикует события в Kafka. Просмотры видео отправляются клиентом батчами в отдельный endpoint сбора событий, чтобы не превращать каждый просмотр в синхронный API-запрос.
...
|--------|--------|------|----------|-----------|---------------|---------------------|
| K8s Worker Node (универсальный) | 120 | 64 | 256 | 200 NVMe | 25 | User, Interaction, Feed, Upload, Counters, Auth (контейнеры) |
...
| L7 Balance Node | 5 | 32 | 64 | 100 | 25 | Nginx (active‑active) |
...
120 * 64 vCPU = 7 680 vCPU установлено. При коэффициенте утилизации 70% доступно 5 376 vCPU для рабочей нагрузки. Суммарный CPU request всех подов (раздел 11.2): 150 * 2 + 150 * 2 + 30 * 2 + 200 * 4 + 400 * 2 + 250 * 2 = 2 760 vCPU. Утилизация по request: 2 760 / 5 376 ≈ 51%, что укладывается в целевые 70%. Реальное потребление обычно ниже request и оценивается примерно в 1 300–1 500 vCPU.

*K8s Worker (High-CPU):*
60 * 96 vCPU = 5 760 vCPU. При 70% доступно 4 032 vCPU. Транскодирование в ДЦ США: 23 млн видео/день * 25% = 5,75 млн видео/сутки. Пиковый час (10% суточного объёма): 5,75 млн * 10% / 3 600 с ≈ 160 видео/с. При использовании аппаратного ускорения и быстрого preset транскодирование одного среднего ролика требует около 16 CPU-секунд: 160 * 16 ≈ 2 556 vCPU в пике. Утилизация: 2 556 / 4 032 = 63% - в пределах нормы.

*L7-балансировщики:*
Пиковый RPS на API Gateway в ДЦ США = 167 666 (из раздела 4.2). Один Nginx-узел (32 ядра, 64 ГБ RAM) выдерживает ≈ 50 000 RPS -> 167 666 / 50 000 = 4 узла. С учётом N+1 = 5 узлов. Утилизация при пике: 167 666 / (5 * 50 000) = 67%. При потере одного узла оставшиеся 4 покроют нагрузку с утилизацией 84% - допустимо.
...
|--------|--------------|-------------|------------------|-------------|
| Auth Service | 150 | 2 / 4 | 4 / 8 | Выдача токенов, валидация JWT/OAuth2, управление сессиями |
| User Service | 150 | 2 / 4 | 4 / 8 | Регистрация, профили пользователей |
| Video Upload Service | 30 | 2 / 4 | 4 / 8 | Выдача pre‑signed URL, приём метаданных |
...
|--------|-------------------|--------------|--------------:|---------|--------------------|
| Auth Service | 167 666 (синхронные API-запросы) | 5 000 | 34 | 150 | Горячий путь; запас на burst и отказоустойчивость |
| User Service | ~25 000 | 1 000 | 25 | 150 | Редкие но тяжёлые запросы (JOIN, запись) |
| Video Upload | ~170 | 500 | 1 | 30 | Минимум мал, но принято 30 реплик для burst и устойчивости к неравномерной активности авторов |
...
Запись идёт асинхронно через Kafka-батчи. Средний fan-out: 23 млн видео/сутки * 50 подписчиков ≈ 1,15 млрд вставок в сутки, то есть ~13 300 QPS глобально. Пик x2.5 ≈ 33 300 QPS глобально, в США около 8 300 QPS. На 20 шардов это ~415 QPS/шард; с batch-фактором ~10 получается около 40 batch-записей/с на шард. Утилизация низкая намеренно, таблица write-heavy в пиковые часы публикаций.
...
**Расчёт CDN-origin трафика**

Пиковый глобальный download-трафик составляет ~1 402 000 Гбит/с, но он распределён по CDN edge/PoP-узлам. Для основного ДЦ США считаем только origin/cache-miss поток: 0,1% от регионального пика 350 500 Гбит/с ≈ 350 Гбит/с. 50 CDN Edge Node с выходом 10 Гбит/с дают до 500 Гбит/с, то есть покрывают origin/refill-трафик с запасом. Массовая отдача пользователям происходит за пределами центрального ДЦ, на распределённых edge-узлах CDN.
...
|-----------|----------------------|-------------------|-----------------|------------|
| K8s Worker универс. (vCPU) | 7 680 vCPU | 5 376 vCPU | ~2 760 vCPU (CPU request) | 51% |
| K8s Worker High-CPU (vCPU) | 5 760 vCPU | 4 032 vCPU | ~2 556 vCPU | 63% |
| L7 Nginx (RPS) | 5 * 50 000 = 250 000 RPS | 175 000 RPS | 167 666 RPS | 67% * |
...
Nginx: при потере одного узла из 5 оставшиеся 4 покрывают 167 666 / (4 * 50 000) = 84% - допустимо. Автомасштабирование не применяется на L7 (bare-metal), поэтому запас намеренно меньше, чем у K8s.
...
**Итого физических серверов в ДЦ США** (без S3) ≈ 537.
Глобально (5 основных ДЦ: США, Европа, Азия, Бразилия, Россия) - около 2 700 серверов без объектного хранилища, с S3 - более 10 000 по всему миру.
