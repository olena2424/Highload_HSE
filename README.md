...
:--- | :---: | :---: | :---:
Трафик отдачи видео (Download) | ~3 155 600 ТБ/сутки | ~1 402 000 Гбит/с | Суточный: 920M DAU * 343 видео * 10 МБ = ~3 155 600 ТБ. Средний трафик: 3 155 600 * 10⁹ байт / 86 400 с * 8 бит = ~292 000 Гбит/с. Пиковый: если 20% суточного объёма приходится на 1 час пик, то пик в 0.20 * 24 = 4.8 раза выше среднего -> ~1 402 000 Гбит/с (1 402 Тбит/с). Этот трафик раздаётся CDN-узлами глобально.
Трафик загрузки видео (Upload) | ~230 ТБ/сутки | ~51 Гбит/с | Суточный: 23 млн видео * 10 МБ = 230 000 ГБ ≈ 230 ТБ. Пиковый: 10% объёма за 1 час -> коэффициент 0.10 * 24 = 2.4x от среднего (~21 Гбит/с) -> ~51 Гбит/с.
...
- При выходе одного узла оставшиеся 2 покрывают расчётные 18,3 Гбит/с (2 * 40 Гбит/с >> 18,3 Гбит/с).
...
| PostgreSQL Standby (Feed_subscription) | 60 | 32 | 256 | 2*1,92 TB NVMe | Синхронная реплика | 3 реплики на шард (N+1 с N≥2) |
| PostgreSQL Read Replica (аналитика, BI) | 8 | 32 | 128 | 2*800 NVMe | Асинхронная реплика | Тяжёлые отчёты |
| Cassandra / ScyllaDB (Likes, Views, Stats) | 30 | 32 | 128 | 2*1,92 TB NVMe | Кольцевой кластер RF=3 | Партиционирование по video_id |
| Redis (кеш + очереди) | 36 | 16 | 128 | 800 GB Optane | Clustered Sentinel | 6 узлов на шард * 6 шардов = 36 узлов |

**Расчёт количества шардов**
- **Users + Follows**: ~4 млрд строк. При 250–300 млн строк на шард -> 16 шардов оптимально, но принимаем 8 шардов с более производительным железом.
- **Video, Comments, Replies**: 25 млрд + ~750 млрд строк -> 25 шардов по ~1 млрд строк.
- **Feed_subscription**: ~92 млрд строк -> 20 шардов по ~1 млрд строк (глобально, ДЦ США ≈ 25% = 5 шардов, таблица показывает полную ёмкость).
- **Cassandra (Likes, Views)**: ~1,8 трлн строк Likes + события просмотров. RF=3, каждый узел хранит до 5 ТБ: 1,8 трлн * 40 байт = 72 ТБ глобально, в США 18 ТБ -> 18/5 * 3 (RF) ≈ 11 узлов, с запасом 30 узлов.
- **Redis**: ~12 ТБ горячих данных. 36 узлов с 200 ГБ/узел (RAM+Optane) = 7,2 ТБ полезных данных. Масштабируется до 60+ узлов при необходимости.
...
Глобально (5 основных ДЦ: США, Европа, Азия, Бразилия, Россия) количество серверов без S3 составит около 3 000, с объектным хранилищем - более 10 000 по всему миру.
