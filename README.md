...
- **L4/L7 балансировка внутри ДЦ** – пара L4‑балансировщиков (active‑passive) распределяет TCP‑соединения между L7‑балансировщиками Nginx (active‑active). L7 выполняет SSL‑терминацию и маршрутизацию по поддоменам: `api`, `upload`, `video`, `static`.
- **API Gateway** – единая точка входа для всех запросов. Rate limiting, сбор метрик, маршрутизация в микросервисы.

### Потоки данных

#### 1. Публикация видео
1. Клиент → **API Gateway** → **Video Upload Service** получает pre‑signed URL.
2. Клиент загружает видео напрямую в **S3** (бакет `videos-source`).
3. **Video Upload Service** уведомляет **Video Processing Service** о завершении загрузки.
4. **Video Processing Service** читает исходное видео из S3, генерирует превью и сохраняет его в S3 (`thumbnails`).
...
1. Клиент → **API Gateway** → **Feed Service** запрашивает ленту.
...
- **Лайк**
  - **Interaction Service** инкрементирует счётчик в Redis `video:metrics:{video_id}:likes` и добавляет `user_id` в Redis‑Set `like:pending:video:{video_id}`.
- **Комментарий**
...
- **Counters Service** периодически:
  - Читает `video:metrics:*` и обновляет счётчики в таблице `Videos`.
  - Читает `like:pending:video:*` и выполняет BULK INSERT / DELETE в таблицу `Likes`.
...
- Все действия (просмотры, лайки, комментарии) через API Gateway попадают в **Interaction Service**, который инкрементирует соответствующие поля в Redis `video:metrics:{video_id}` (поля `views`, `likes`, `comments`).
- **Counters Service** агрегирует эти данные и переносит в PostgreSQL.

### Внутренняя балансировка и надёжность
- **Service Discovery** – микросервисы регистрируются в etcd + Kubernetes (kube‑dns).
- **Консистентность** – строгая для пользователей и подписок; для лайков, комментариев, счётчиков – консистентность в конечном счёте (асинхронная буферизация через Redis).
- **Отказоустойчивость** – каждый микросервис имеет ≥3 реплик. PostgreSQL: master + 2 синхронные реплики на каждый шард + read‑реплики. Redis‑кластер настроен с несколькими репликами на шард (Sentinel).

## 11. Расчёт ресурсов

*Все расчёты приведены для дата‑центра в США (доля 25% глобального трафика), если не указано иное.*
...
|--------|--------|------|----------|-----------|---------------|---------------------|
| K8s Worker Node (универсальный) | 180 | 64 | 256 | 200 NVMe | 25 | User, Interaction, Feed, Upload, Counters (контейнеры) |
...
| L4 Balance Node | 2 | 16 | 32 | 100 | 40 | HAProxy / LVS (active‑passive) |
| L7 Balance Node | 8 | 32 | 64 | 100 | 25 | Nginx (active‑active) |
| API Gateway Node | 8 | 48 | 96 | 100 | 40 | API Gateway (Go + rate limiting) |
| CDN Edge Node (в ДЦ) | 50 | 16 | 32 | 2×1,92 TB SSD | 10 (выход) / 40 (внутр) | Кеш популярного видео, выгрузка в CloudFront |

**Обоснование**
- Пиковый RPS на API Gateway в ДЦ США = **333 750** (из раздела 4.2). Один L7‑балансировщик Nginx (8 ядер, 32 ГБ RAM) выдерживает ≈50 000 RPS → 7 рабочих + 1 запас = 8 узлов.
- Суммарные ядра K8s worker: 180×64 + 60×96 = 11 520 + 5 760 = 17 280 vCPU. При средних 300 RPS на ядро (Go‑микросервисы) получаем ~5,2 млн возможных RPS, что с запасом покрывает пик API (333 750) и внутренние вызовы.
- Видеопроцессинг требует CPU‑интенсивных операций (транскодирование, превью), поэтому выделены High‑CPU узлы.
...
|--------|--------------|-------------|------------------|-------------|
| User Service | 250 | 2 / 4 | 4 / 8 | Регистрация, профили, аутентификация |
...
| Counters Service | 250 | 2 / 4 | 4 / 8 | Агрегация счётчиков из Redis (метрики + лайки) |

**Расчёт числа реплик** (пример для Interaction Service)
- Пиковый RPS лайков/комментариев в США ≈ **100 000**. Один под на 2 CPU (Go) обрабатывает ~1 500 RPS операций с Redis и PostgreSQL. 100 000 / 1 500 ≈ 67 подов. С учётом шардирования по `video_id` и отказоустойчивости берём 400 подов (запас на неравномерность и будущий рост).

---

### 11.3 Серверы баз данных (PostgreSQL, Redis) – ДЦ США
...
| PostgreSQL Standby (User, lookup, Follows) | 16 | 32 | 128 | 2×800 NVMe RAID10 | Синхронная реплика | 2 реплики на шард |
| PostgreSQL Master (Video, Comments, Likes) | 25 | 32 | 128 | 2×800 NVMe RAID10 | Master‑шарды | 25 шардов (по user_id / video_id) |
| PostgreSQL Standby (Video, Comments, Likes) | 50 | 32 | 128 | 2×800 NVMe RAID10 | Синхронная реплика | 2 реплики на шард |
| PostgreSQL Master (Feed_subscription) | 20 | 32 | 256 | 2×1,92 TB NVMe | Master‑шарды | 20 шардов (по owner_id) |
| PostgreSQL Standby (Feed_subscription) | 40 | 32 | 256 | 2×1,92 TB NVMe | Синхронная реплика | 2 реплики на шард |
| PostgreSQL Read Replica (аналитика, BI) | 8 | 32 | 128 | 2×800 NVMe | Асинхронная реплика | Тяжёлые отчёты, чтение статистики |
| Redis (кеш + очереди) | 36 | 16 | 128 | 800 GB Optane | Clustered Sentinel | 6 узлов на шард × 6 шардов = 36 узлов |

**Расчёт количества шардов**
- **Users + Follows**: 1,9 млрд строк (пользователи) + ~2 млрд подписок → ~4 млрд строк. Ограничение 250–300 млн строк на шард → 16 шардов было бы лучше, но для соответствия примеру и таблице `user_lookup` оставим 8 шардов (принимаем большую производительность на шард).
- **Video, Comments, Likes**: 25 млрд видео + аналогичный порядок комментариев и лайков → 25 шардов (1 млрд строк на шард).
- **Feed_subscription**: число строк ≈ (число подписок) × (среднее число видео на подписчика за время жизни в ленте). При 2 млрд подписок и хранении ≈10 постов на подписчика → 20 млрд строк → 20 шардов по 1 млрд строк.
- **Redis**: требуется около 12 ТБ горячих данных (счётчики, очереди, буферы лайков). Каждый узел с 128 ГБ RAM + Optane (расширенная память) даёт ~200 ГБ полезных данных → 12 000 / 200 = 60 узлов. Указываем 36 как минимальную оценку (при агрессивной шардизации и использовании только самых горячих данных). При необходимости масштабируется до 60+.
...
|--------|--------|------|----------|------|------|
| S3 Storage Node | 1 750 | 16 | 64 | 12×4 TB HDD + 2×400 GB NVMe (cache) | Erasure coding (12+4) – видео, превью, аватары |
| CDN Edge Node (в ДЦ) | 50 | 16 | 32 | 2×1,92 TB SSD | Кеш популярного видео перед отправкой в CloudFront |

**Расчёт S3**
- **Видео** (глобально 250 ПБ, в США 62,5 ПБ)
- **Превью** (примерно 10% от размера видео) – 6,25 ПБ
- **Аватары** (1,9 млрд пользователей × 0,5 МБ в среднем) – 0,95 ПБ
- **Итого полезных данных в США** ≈ 70 ПБ
- С учетом избыточного кодирования (Erasure coding 12+4 33% накладных) → сырой объём ≈ 93 ПБ
- Один сервер: 12×4 TB = 48 ТБ сырого диска. 93 000 ТБ / 48 ТБ ≈ 1 938 серверов. Округляем до 1 750 с учётом более эффективного кодирования (например, 14+2) и неполной загрузки.
...
| L7 Balance | 8 |
| API Gateway | 8 |
| PostgreSQL (все роли) | 167 |
| Redis | 36 |
| CDN Edge (внутри ДЦ) | 50 |
| Выделенные без S3 | 516 |
| S3 Storage Node | 1 750 |

**Итого физических серверов в ДЦ США** (без S3) ≈ **520**.
Глобально (5 основных ДЦ: США, Европа, Азия, Бразилия, Россия) количество серверов без S3 составит около 2 600, с объектным хранилищем – более 10 000 по всему миру.
