...
ScyllaDB / Cassandra | Счётчики вовлечённости (лайки/просмотры/репосты), истории активности (опционально, для разгрузки PostgreSQL) | Для счётчиков критичны сверхнизкие P99-задержки и высокая пропускная способность на запись. В текущей схеме счётчики денормализованы в таблице Videos PostgreSQL, однако при росте нагрузки может потребоваться вынос в ScyllaDB.
...
ScyllaDB / Cassandra (счётчики, ленты) | Репликация (RF=3), распределение по зонам (rack‑aware) | Клиенты переключаются на доступные реплики | RTO < 1 с, RPO < 1 с
