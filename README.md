...
:--- | :--- | :--- | :---
User | Пользователи сервиса | id (PK), username (UK), email (UK), avatar_url, password_hash, bio, created_at, uploads (счётчик загрузок) | Один ко многим с Video (author), Follow (follower/followee), Comment, FeedEntry (owner)
Follow | Подписки (кто на кого подписан) | follower_id (FK → User.id), followee_id (FK → User.id), created_at | Составной PK (follower_id, followee_id); многие ко многим через себя
Video | Метаданные видео | id (PK), title, description, duration_seconds, status, created_at | Связана с User (автор), Comment, VideoMetric, FeedEntry
FeedEntry | Персональная лента пользователя (прекомпьютированная) | owner_id (FK → User.id), video_id (FK → Video.id), author_id (FK → User.id), published_at | Составной PK (owner_id, video_id); денормализованная таблица для быстрой выдачи ленты
Comment | Комментарии к видео | id (PK), video_id (FK → Video.id), user_id (FK → User.id), text, like_count, created_at | Один ко многим: видео → комментарии
VideoMetric | Счётчики и метрики видео (лайки, просмотры, репосты) | video_id (PK, FK → Video.id), likes, views, shares, updated_at | Один к одному с Video; часто обновляется

Сами видеофайлы, аватары пользователей, превью хранятся в объектном хранилище. В БД сохраняются только метаданные и URL-ссылки на эти файлы.
...
FeedEntry | 920 млн * среднее число видео в ленте | Лента содержит ~200 записей на пользователя -> ~184 млрд записей (кешируется, может быть ограничена по времени)
Comment | ~500 млрд | В среднем 20 комментариев на видео (25 млрд * 20)
VideoMetric | 25 млрд | Одна запись на видео
...
Follow | 32 байта | два FK (по 16) + timestamp (8) = 40, сжатие ~32
Video | 150 байт | UUID(16) + title(64) + description(128) + int(4) + status(1) + timestamp(8) ≈ 221, после сжатия ~150
FeedEntry | 48 байт | три FK (3×16) + timestamp(8) = 56 → ~48
Comment | 200 байт | UUID(16) + FK(16) + FK(16) + text(256) + int(4) + timestamp(8) ≈ 316 → ~200 (текст может быть длиннее)
VideoMetric | 48 байт | FK(16) + три счётчика (3×8) + timestamp(8) = 48
...
Follow | 100 | 32 | 3 200
Video | 25 | 150 | 3 750
FeedEntry | 184 | 48 | 8 832
Comment | 500 | 200 | 100 000
VideoMetric | 25 | 48 | 1 200
Всего | | | ~117 362

### Нагрузка на чтение/запись (QPS)
Операция | Таблицы | Тип | Средний QPS | Пиковый QPS (x2.5)
:--- | :--- | :--- | :--- | :---
Просмотр ленты (получение списка видео) | FeedEntry (чтение), Video (чтение) | Чтение | 98 000 | 245 000
Просмотр видео (метаданные) | Video, VideoMetric (чтение) | Чтение | 3 655 000 | 9 137 500
Публикация видео | Video (вставка), FeedEntry (вставка в ленты подписчиков) | Запись | 266 000 | 665 000
Лайк видео | VideoMetric (обновление), VideoMetric – запись | Запись | 160 000 | 400 000
Комментарий | Comment (вставка) | Запись | ~50 000 | 125 000
Подписка/отписка | Follow (вставка/удаление) | Запись | ~10 000 | 25 000
Сбор статистики (просмотры) | VideoMetric (обновление счётчика views) | Запись | ~3 655 000 | 9 137 500

Итого:
- Чтение ~3.75 млн QPS в среднем, до 9.4 млн QPS в пик
- Запись ~4.1 млн QPS в среднем, до 10.3 млн QPS в пик

### Требования к консистентности
Сущность / операция | Требование | Обоснование
:--- | :--- | :---
Регистрация, смена пароля, подписка | Strong consistency (линейная) | Безопасность и целостность данных пользователя. Нельзя, чтобы подписка пропала после подтверждения
Лайки, просмотры, счётчики | Eventual consistency (слабая) | Допустима задержка обновления счётчиков (несколько секунд). Высокая нагрузка требует асинхронной обработки
Лента (FeedEntry) | Eventual consistency с гарантией доставки в течение 1-2 секунд | Новое видео может появиться в ленте не мгновенно, но должно появиться у всех подписчиков
Комментарии | Strong consistency для только что опубликованного комментария (чтобы автор видел его сразу), eventual для остальных | Важно для UX: пользователь ожидает увидеть свой комментарий сразу
Метаданные видео (название, описание) | Strong consistency | Изменения редки, важна целостность
...
:--- | :--- | :---
User | id (UUID) или username | Равномерное распределение, частые запросы по id и username
Follow | follower_id | Чтобы все подписки одного пользователя лежали на одном шарде – удобно для проверки подписки и выдачи списка
Video | author_id | Видео одного автора на одном шарде – упрощает выборку видео пользователя
FeedEntry | owner_id | Критично: лента пользователя целиком на одном шарде, чтобы читать её без cross-shard запросов
Comment | video_id | Все комментарии к видео на одном шарде – эффективная загрузка комментариев под видео
VideoMetric | video_id | Обновление счётчиков происходит по одному видео – локально

Особенности распределения нагрузки:
- При записи в ленту (FeedEntry) при публикации видео нужно вставить записи в ленты всех подписчиков. Это операция, которая создаёт множество записей на разных шардах (по owner_id подписчиков). Требует асинхронной обработки через очереди.
- Чтение ленты - очень высокая нагрузка (98k RPS). Благодаря шардированию по owner_id, каждый запрос идёт ровно в один шард.
- Обновление счётчиков (VideoMetric) - запись по ключу video_id – высокая интенсивность (миллионы RPS). Используется буферизация в памяти и пакетная запись.
<!--
Кеширование:
- Лента пользователя кешируется в Redis (TTL 5-10 минут), ключ feed:{owner_id}.
- Популярные видео и их метрики кешируются в CDN и in-memory кеше (Memcached).
- Счётчики лайков/просмотров могут храниться в Redis и периодически синхронизироваться с БД.

Буферы:
- Для записи ленты (FeedEntry) используется очередь (Kafka). Публикация видео порождает сообщения, которые консьюмеры обрабатывают и рассылают записи в шарды БД.
- Для счётчиков (VideoMetric) – буфер в памяти каждого сервера (например, раз в секунду отправка пачки обновлений). -->
